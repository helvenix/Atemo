...
import { createEventSchema } from "../type";

import { Calendar as CalendarIcon } from "lucide-react";

export function AddEvent({setDialog} : {setDialog: React.Dispatch<React.SetStateAction<boolean>>}){
    const { addEvent } = useEvents()

    type EventFormValue = z.infer<typeof createEventSchema>
    ...
    const handleAdd = async (event: EventFormValue) => {
        try{
            setDialog(false)
            const res = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/events/`, 
                event,
                { withCredentials: true }
            )
            addEvent(res.data.event);
            toast.success("Event has been succesfully added");
        }catch(e: any){
