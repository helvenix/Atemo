...
function MarkAsDoneHandler({item, urgent, setHovered}: Pick<FocusCardProps<Task>, "item" | "urgent" | "setHovered">){
    const { updateTask } = useTasks()
    const handleMarkAsDone = async (item: Task) => {
        try{
            const res = await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/items/${item._id}/done`,
                { withCredentials: true }
            )
            updateTask(res.data.item);
...
function DeleteHandler({item, setHovered}: Pick<FocusCardProps<Task>, "item" | "setHovered">){
    ...
    const handleDelete = async (item: Task) => {
        try{
            const res = await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/tasks/${item._id}`,
                { withCredentials: true }
            )
            removeTask(item._id)
